...
static void setGpioConfiguration(uint8_t host) {
  int netFn = 0x38;
  int cmd = 0x6;
  std::vector<uint8_t> cmdData{0x15, 0xa0, 0x0, 0x03, 0x0, 0x0,
                               0x0,  0x0,  0x0, 0x12, 0x12};
  std::vector<uint8_t> respData;
...
static int bic_ipmb_wrapper(uint8_t host, uint8_t netFn, uint8_t cmd,
  ...
  while (retry < 3) {
    ...
    if (respData.empty()) {
      ...
      if (!is_bic_ready(host)) {
        std::cout << "BIC is busy in host :" << host << std::endl;
        break;
      }
  ...
  if (DEBUG) {
    std::cout << "BIC resp data from : " << host << std::flush;
    for (auto d : respData) {
      std::cout << std::hex << int(d) << " " << std::flush;
    }
    std::cout << "\n" << std::flush;
  }
...
static int bic_get_config(uint8_t host, bic_config_t *cfg) {
  ...
  // std::vector<uint8_t> cmdData = {0x15, 0xA0, 0x00}; // IANA ID
  std::vector<uint8_t> respData;
  uint8_t rlen = 0;
  ...
  sendIPMBRequest(host, NETFN_OEM_1S_REQ, CMD_OEM_1S_GET_CONFIG, cmdData,
                  respData);
  // ret = bic_ipmb_wrapper(host, NETFN_OEM_1S_REQ, CMD_OEM_1S_GET_CONFIG,
  //                         cmdData, 0x03, respData, &rlen);

  std::cerr << "bic_get_config Response:\n" << std::flush;
  for (int i = 0; i < respData.size(); i++) {
    printf("%2x :", respData[i]);
  }

  printf("\n");

  //*(uint8_t *)cfg = respData[3];
...
static int bic_set_config(uint8_t host, bic_config_t *cfg) {
  ...
  std::cout << "bic_set_config for host:" << host << std::endl;

  sendIPMBRequest(host, NETFN_OEM_1S_REQ, CMD_OEM_1S_SET_CONFIG, cmdData,
                  respData);
  // ret = bic_ipmb_wrapper(host, NETFN_OEM_1S_REQ, CMD_OEM_1S_SET_CONFIG,
  //                          cmdData, 0x04, respData, &rlen);

  std::cerr << "bic_set_config Response :\n" << std::flush;
  for (int i = 0; i < respData.size(); i++) {
    printf("%x:", respData[i]);
  }
  std::cout.flush();
...
static int post_enable(uint8_t host) {
  ...
  std::cout << "post_enable for host :" << host << std::endl;
...
// Read POST Buffer
static int bic_get_post_buf(uint8_t slot_id, uint8_t *buf, uint8_t *len) {
  std::vector<uint8_t> cmdData = {0x15, 0xA0, 0x00}; // IANA ID
  std::vector<uint8_t> respData;
  uint8_t rlen = 0;
  int ret;
  ...
  *buf = respData[3];
  ...
  std::cerr << "bic_get_post_buf Response :\n" << std::flush;
  for (int i = 0; i < respData.size(); i++) {
    printf("%x:", respData[i]);
  }
  std::cout.flush();
...
static int post_get_last(uint8_t slot, uint8_t *status) {
...
static int pal_get_hand_sw_physically(uint8_t *pos) {
  ...
  loc =
      (((*(volatile uint32_t *)m_gpio_hand_sw) >> 20) & 0xF) % 5; // GPIOAA[7:4]
...
void debug_card_handler(uint8_t val, uint8_t pos) {
...
static void readPostcode(uint8_t val, uint8_t pos) {
  switch (pos) {
  case 1:
...
int main(int argc, char *argv[]) {
  ...
  fb_ipmi::miscIface->register_method("updatePostcode", fb_ipmi::readPostcode);
